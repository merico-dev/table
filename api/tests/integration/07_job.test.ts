...
describe('JobService', () => {
  ...
  describe('addRenameDataSourceJob', () => {
    it('add several jobs', async () => {
      const job1 = await JobService.addRenameDataSourceJob({ type: 'postgresql', old_key: 'pg', new_key: 'pg_renamed' });
      ...
      const job2 = await JobService.addRenameDataSourceJob({ type: 'http', old_key: 'jsonplaceholder', new_key: 'jsonplaceholder_renamed' });
      ...
      const job3 = await JobService.addRenameDataSourceJob({ type: 'non_existent', old_key: 'old_key', new_key: 'new_key' });
  ...
  describe('list', () => {
    it('no filters', async () => {
      const results = await jobService.list(undefined, { field: 'create_time', order: 'ASC' }, { page: 1, pagesize: 20 });
      ...
      expect(results.data[4].result['error'].message).toContain('Could not find any entity of type "DataSource" matching');
      ...
      expect(results.data[7].result['error'].message).toContain('Could not find any entity of type "DataSource" matching');
    ...
    it('with SUCCESS search filter', async () => {
      const results = await jobService.list({ search: 'SUCCESS' }, { field: 'create_time', order: 'ASC' }, { page: 1, pagesize: 20 });
      expect(results).toMatchObject({
        total: 6,
        offset: 0,
        data: [
    ...
    it('with FAILED search filter', async () => {
      const results = await jobService.list({ search: 'FAILED' }, { field: 'create_time', order: 'ASC' }, { page: 1, pagesize: 20 });
      expect(results).toMatchObject({
        total: 2,
        offset: 0,
        data: [
      ...
      expect(results.data[0].result['error'].message).toContain('Could not find any entity of type "DataSource" matching');
      ...
      expect(results.data[1].result['error'].message).toContain('Could not find any entity of type "DataSource" matching');
