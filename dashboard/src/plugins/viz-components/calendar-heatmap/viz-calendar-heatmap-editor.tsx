...
export function VizCalendarHeatmapEditor({ context }: VizConfigProps) {
  ...
  const conf: ICalendarHeatmapConf = useMemo(() => _.defaultsDeep({}, confValue, DEFAULT_CONFIG), [confValue]);

  const { control, handleSubmit, watch, getValues, reset } = useForm<ICalendarHeatmapConf>({ defaultValues: conf });
  useEffect(() => {
    reset(conf);
  }, [conf]);
    ...
    <Stack spacing="xs">
      <form onSubmit={handleSubmit(setConf)}>
        <Group position="left" py="md" pl="md" sx={{ borderBottom: '1px solid #eee', background: '#efefef' }}>
          <Text>CalendarHeatmap Config</Text>
