...
export function Filters({ context, submit }: IFilters) {
  const [timeRange, setTimeRange] = React.useState<TimeRangeType>([new Date(2021, 1, 1), new Date()]);
  ...
  const hasChanges = React.useMemo(() => {
    return (
      !_.isEqual(timeRange, context.timeRange) ||
      !_.isEqual(emails, context.emails) ||
      !_.isEqual(repoIDs, context.repoIDs) ||
      !_.isEqual(jenkinsJobIDs, context.jenkinsJobIDs)
    );
    ...
    <Group position="apart" p="md" mb="md" sx={{ boxShadow: '0px 0px 10px 0px rgba(0,0,0,.2)' }}>
      <Group align="flex-start">
        <DateRangePicker
          ...
          sx={{ width: 240 }}
        />
        <ContributorSelector value={emails} onChange={setEmails} />
        <RepositorySelector value={repoIDs} onChange={setRepoIDs} />
        <JenkinsJobSelector value={jenkinsJobIDs} onChange={setJenkinsJobIDs} />
      </Group>
      <Group sx={{ alignSelf: 'flex-end' }}>
        <Button disabled={!hasChanges} color="blue" size="sm" onClick={doSubmit}>
          Submit
        </Button>
      </Group>
    </Group>
