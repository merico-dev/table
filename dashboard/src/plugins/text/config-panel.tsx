...
} from '../../panel/settings/common/mantine-color';
import {
  MantineFontWeightSlider
} from '../../panel/settings/common/mantine-font-weight';
import { VizConfigProps } from '../../types/plugin';
import { useStorageData } from '../hooks/use-storage-data';
import { CONF, IParagraph, ITextViewProps } from './types';
...
export const TextConfig = (props: VizConfigProps) => {
  const {
    value: conf,
    set: setConf
  } = useStorageData<ITextViewProps>(props.context.instanceData, CONF);
  if (!conf) {
    return null;
  }
  const form = useForm({
    ...
    <Group direction="column" mt="md" spacing="xs" grow>
      <form onSubmit={form.onSubmit(setConf)}>
        ...
        <Group position="apart" mb="xs"
               sx={{ ' + .mantine-Group-root': { marginTop: 0 } }}>
        ...
        {form.values.paragraphs.map((item, index) => (
          <Group key={index} direction="column" grow my={0} p="md" pr={40}
                 sx={{ border: '1px solid #eee', position: 'relative' }}>
            <TextInput
            ...
            <Group position="apart" grow
              ...
              <MantineFontWeightSlider
                label="Font Weight" {...form.getListInputProps('paragraphs', index, 'weight')} />
            </Group>
            <ActionIcon
              color="red"
              variant="hover"
        ...
        <Prism language="json" colorScheme="dark"
               noCopy>{JSON.stringify(form.values, null, 2)}</Prism>
      </form>
    </Group>);
};
