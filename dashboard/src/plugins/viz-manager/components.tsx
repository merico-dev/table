...
export type IViewComponentProps<TDebug = {}> =
  { panel: IViewPanelInfo, data: any, vizManager: IVizManager }
  & TDebug;
export const VizViewComponent = <T, >(props: IViewComponentProps<T>) => {
  ...
  return <Comp context={context}
               instance={instance}
               {...omit(props, ['panel', 'vizManager', 'data'])}/>;
};
type IConfigComponentProps<TDebug = {}> =
  { panel: IPanelInfo, panelInfoEditor: IPanelInfoEditor, vizManager: IVizManager, data: any }
  & TDebug;
export const VizConfigComponent =
  <T, >(props: IConfigComponentProps<T>) => {
    const {
      vizManager,
      panel,
      panelInfoEditor,
    ...
    return <Comp context={context}
                 instance={instance} {...omit(props, ['panel', 'vizManager', 'data', 'panelInfoEditor'])}/>;
  };
