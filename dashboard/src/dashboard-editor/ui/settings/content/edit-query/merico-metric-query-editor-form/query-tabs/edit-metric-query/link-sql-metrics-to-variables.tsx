...
import { SqlVarSelector } from './sql-var-selector';
...
export const LinkSqlMetricsToVariables = observer(({ queryModel }: Props) => {
  ...
  if (mmInfo.metricDetail.metricType !== 'sql') {
    return null;
  }
  ...
  return (
    <Stack gap={7}>
      <Group justify="flex-start" gap={8}>
        <Text size="sm">看板变量与 SQL 指标关联</Text>
        <Tooltip label="将看板的筛选器参数与指标的查询参数关联上，使指标可以按照看板的筛选条件和背景变量进行关联展示。">
      ...
      <Table withTableBorder withColumnBorders layout="fixed" styles={MetricTableStyles}>
        ...
        <Table.Thead>
          <Table.Tr>
            <Table.Td>SQL 查询参数</Table.Td>
            <Table.Td>看板变量</Table.Td>
            <Table.Td colSpan={2}>变量值为真时运行查询</Table.Td>
          </Table.Tr>
        </Table.Thead>
        <Table.Tbody>
          {config.sqlVariables.map((f) => (
            <Table.Tr key={f.sqlVar}>
              <Table.Td pr={0}>
                <SqlVarSelector
                  mmInfo={mmInfo}
                  value={f.sqlVar}
                  onChange={f.setSqlVar}
                  usedSqlVariables={config.usedSqlVariables}
                />
              </Table.Td>
          ...
          <Table.Tr className="add-a-row">
            <Table.Td pr={0}>
              <SqlVarSelector
                mmInfo={mmInfo}
                value={null}
                onChange={(v: string | null) => v && config.addSqlVariable(v, '')}
                usedSqlVariables={config.usedSqlVariables}
              />
            </Table.Td>
            <Table.Td colSpan={2} pr={0}>
