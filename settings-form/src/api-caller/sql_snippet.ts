...
export const sql_snippet = {
  list: async ({ pagination }: ListSQLSnippetReqType, signal?: AbortSignal): Promise<ListSQLSnippetRespType> => {
    const resp: ListSQLSnippetRespType = await APIClient.post(signal)('/sql_snippet/list', {
      sort: [
        {
          field: 'update_time',
          order: 'DESC',
  ...
  get: async (id: string, signal?: AbortSignal): Promise<SQLSnippetDBType | null> => {
    const resp: SQLSnippetDBType = await APIClient.post(signal)('/sql_snippet/get', {
      id,
    });
    return resp;
  },
  create: async (payload: TCreateSQLSnippetPayload, signal?: AbortSignal): Promise<SQLSnippetDBType> => {
    const resp: SQLSnippetDBType = await APIClient.post(signal)('/sql_snippet/create', payload);
    return resp;
  },
  update: async (payload: TUpdateSQLSnippetPayload, signal?: AbortSignal): Promise<SQLSnippetDBType> => {
    const resp: SQLSnippetDBType = await APIClient.put(signal)('/sql_snippet/update', payload);
    return resp;
  },
  delete: async (id: string, signal?: AbortSignal): Promise<void> => {
    return APIClient.post(signal)('/sql_snippet/delete', { id });
  },
};
